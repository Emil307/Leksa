...
Location: `frontend/src/features/{feature}/logic/test/{feature}.logic.test.ts`

```typescript
import { describe, it, expect } from 'vitest'
import { validateEmail, validatePassword, isFormValid, buildRegistrationRequest } from '../main/registration.logic'
...
Create minimal stub in `logic/main/{feature}.logic.ts`:
...
Create the frozen interface file in `types/` (re-exported from `types/index.ts`) if not present:
