...
- Use **exact values** from mockup, not Tailwind approximations -- but written in `rem` (`1rem = 16px`): mockup `padding: 12px 16px` becomes `py-[0.75rem] px-[1rem]`, `font-size: 15px` becomes `text-[0.9375rem]`. Only `1px`/`2px` borders and media-query breakpoints stay in `px`.
