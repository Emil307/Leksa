...
- **Split stacked acceptance scenarios into readable tests.** RED must not copy a large or multi-phase spec scenario into one large test method. Write one small, focused test case per independent execution phase or coherent outcome, visibly grouped under the spec scenario. The group still owns one `progress.md` block and one Stage 2 cycle.
- **Each acceptance case stands alone; the group preserves coverage.** Give every case its own context and setup, with no ordering or shared mutable state. Together the cases map every scenario Then; splitting may duplicate setup, never delete or weaken an assertion. Predict and observe every case separately, keep already-green cases enabled, and disable failing cases only after their predictions match.
...
- **green-acceptance and green-selenium are remove-marker-only phases.** The ONLY allowed change is removing the scenario target's test disable/skip marker or markers. For a multi-case acceptance target, enable and verify the whole group together. No production code changes (backend or frontend), no Statements changes, no new files. If any case does not pass, STOP and report — the missing implementation belongs in an earlier phase (green-usecase, green-adapter, align-design) that was incomplete or skipped. Never "fix" backend code in a green-acceptance/green-selenium step.
...
- **Never re-run tests to get failure details you already have.** The output file from a completed run contains full stack traces. Read that output file directly — don't re-run the suite or even individual tests just to see the error. Only re-run a test when you've changed code and need to verify the fix.
...
- **Stop on first failure — then investigate immediately.** When running acceptance suites, use the platform's persistent-command capability and poll the output for failure/success markers. When the first failure appears: (1) stop the running process through the platform's process-control capability, (2) **tell the user explicitly**: "Stopped after the first failure. N tests passed, 1 failed, M tests did not run — there may be additional failures in the remaining tests." (3) read the stack trace from the output, (4) investigate the root cause. Do NOT continue reading more failures, do NOT wait for completion after the first failure, and do NOT collect a summary of all failures. The first failure is the only one that matters — fix it before re-running. Waiting wastes minutes while the answer is already in front of you. **Never present a stopped-early run as if it were a complete run.** The user must always know how many tests were skipped.
- **Show progress while tests run.** Never block silently waiting for test completion. A single shell call with a polling loop does NOT show progress — output may appear only when the command finishes. Instead, make **separate short polls** through the platform's process-control capability, so each result is displayed to the user immediately. The user must always have visibility into what is happening — silent waits erode trust.
...
### Test Through Architectural Entries
- A layer test targets an architectural entry, not every production type in the layer. Request/response objects, mappings, persistence models, framework helpers, security collaborators, resolvers, and exception translators are implementation details exercised through that entry; do not give them standalone tests merely because the design creates or changes them.
- **Usecase tests enter through public usecase methods.** Compose the application layer as production does, replacing third-layer ports with functional Fakes. Setup may call one public usecase and the action/assertion another; domain objects, requests, responses, helpers, and cross-usecase flow are covered through those methods.
- **Adapter tests enter through the adapter's public technology-facing boundary.** For first-layer adapters, drive the controller, listener, filter, or equivalent inbound entry and observe the complete adapter behavior. For third-layer adapters, call the implemented port boundary. DTO conversion, authentication plumbing, exception translation, persistence entities, mappings, and other collaborators remain inside that mini-application boundary.
- A changed internal type still belongs in the owning entry's focused test surface and coverage scope. Add a separate test target only when the type is itself a public architectural entry with independently observable behavior.
...
- **Test description = spec text.** A single-case target carries the spec scenario verbatim. In a multi-case acceptance target, the group carries the scenario title and every method carries a self-contained Given/When/Then slice derived from that scenario; never begin a case with context available only from a sibling case. The method name remains a short identifier and the display annotation remains the readable specification.
...
**Prefer recursive/structural comparison over manual per-field assertions.** For immutable data objects, use the test framework's recursive comparison to get pinpoint field-by-field diffs in one line — no need for shared assertion utility classes with many per-field assertions. Reserve per-field assertions only when you need custom comparators or field exclusions. Settle what to compare, and which fields to exclude, by reading the production code — never by copying how another test asserts: a test that checks one field is under-asserting, not a spec for the rest.
