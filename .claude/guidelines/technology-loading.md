...
The active technology profile is declared in `ProductSpecification/technology.md` via the `tech-profile:` block. The profile is **composite** — five independent concern keys, each pointing to a separate tech profile directory.
...
| Bot handlers, logic, API clients, middleware | `bot` |
| Acceptance tests (bot E2E) | `bot` |
...
1. **Read profile**: Read `ProductSpecification/technology.md`, extract all five concern keys from the `tech-profile:` block
...
2. Update `ProductSpecification/technology.md` to set the five concern keys
