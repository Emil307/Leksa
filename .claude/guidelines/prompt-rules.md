...
- **The upstream merge surface is the prompt library only** — the `.claude/` tree and the always-on project instruction file. Product specification, product code, infrastructure, and repo-level docs are never taken from upstream: its copies are template placeholders, and overwriting them destroys the product. A framework change may define an explicit, deterministic local-data migration; `/framework-sync` runs that after the prompt merge in a separate commit, never by copying upstream product files.
...
- Test execution commands → **test-runner.md** agent only; skills/agents use the platform's named skill execution capability
