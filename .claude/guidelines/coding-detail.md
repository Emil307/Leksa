...
- Prefer code that reveals intent through names, cohesive methods, and explicit
  abstractions. Source files carry no comments and no docstrings at all — the
  hard rule and its two exemptions are in `.claude/rules/coding-rules.md`
  ("No Comments"). Reaching for a comment means the code needs a better name, a
  smaller method, or an extracted type; make that change instead. Durable
  rationale goes to the story's `decisions/*-decision.md`, `endpoints.md`, or
  `interview.md`, never into the source.
