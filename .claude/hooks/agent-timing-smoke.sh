...
# Codex dispatch shape: task_name replaces subagent_type and agent_id uses
# snake_case in the response. Top-level caller id may be absent.
run_case "codex-return-shape" return \
  '{"session_id":"smoke-session","tool_use_id":"tu_codex",
    "tool_input":{"task_name":"premortem-agent","message":"full prompt"},
    "tool_response":{"agent_id":"codex-agent-id"}}' \
  '.event=="return" and .agent_id=="codex-agent-id" and .caller_id==""
   and .agent_type=="premortem-agent" and .description=="premortem-agent"'
