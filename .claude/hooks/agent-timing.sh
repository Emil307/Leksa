...
# Registered for Claude in .claude/settings.json and for Codex in
# .codex/hooks.json on agent dispatch/return and subagent lifecycle events.
...
#   caller_id    tool events only: the dispatching agent's id when the host
#                includes it (Claude does; Codex currently may not)
...
if [[ ! "$ts" =~ \.[0-9]{3}Z$ ]]; then
  if command -v python3 >/dev/null 2>&1; then
    ts="$(python3 -c 'from datetime import datetime, timezone; print(datetime.now(timezone.utc).isoformat(timespec="milliseconds").replace("+00:00", "Z"))')"
  else
    # BSD date does not support %N. Preserve a valid millisecond-shaped UTC
    # timestamp even on a minimal host without Python.
...
jq -cn --arg ts "$ts" --arg event "$event" --arg session "$session" --argjson in "$input" '{
  ...
  agent_id: (if ($event | startswith("subagent_"))
             ...
             else ($in.tool_response.agentId? // $in.tool_response.agent_id?
                   // $in.tool_response.id? // "") end),
  caller_id: (if ($event | startswith("subagent_")) then ""
              else ($in.agent_id // "") end),
  agent_type: ($in.tool_input.subagent_type // $in.tool_input.task_name
               // $in.agent_type
               // $in.subagent_type // "unknown"),
  description: ($in.tool_input.description // $in.tool_input.task_name
                // $in.description // "")
