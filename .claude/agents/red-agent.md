...
10. **Post-implementation trivial-test gate (frontend-logic, frontend-api, and bot-logic only)** — review every assertion in the test just written. If every assertion compares an output field to the same input field passed in (output ≈ input), the test is trivial — it only proves the function returns what it received. **STOP.** Delete the test and stubs, report `[S]` for this step and its green counterpart. This catches cases where step 4 misjudged.
...
| Layer | Template Path |
|-------|---------------|
| bot-logic | `.claude/tech/{bot}/templates/logic/test-class.md` |
| bot-api | `.claude/tech/{bot}/templates/api/test-class.md` |
| bot-handler | `.claude/tech/{bot}/templates/telegram/test-class.md` |
| bot-acceptance | `.claude/tech/{bot}/templates/acceptance/test-class.md` |
...
Bot acceptance is isolated and dispatcher-driven. Do not start the deployed bot or
borrow the API application's environment; use the strict API stub declared by the
bot profile and predict the user-visible bot failure.
