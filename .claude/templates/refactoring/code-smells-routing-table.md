...
| Same utility set repeated in 2+ components | Extract a shared component | `extract-shared-ui.md` |
| Same utility set repeated within one component | Extract a `const` above the component | (inline -- just extract) |
...
| Raw color/font as an arbitrary utility value (`bg-[#hex]`, `font-['Name']`) | Add a token to the theme `@theme` block; use the named utility | `colocate-component-styles.md` |
| `px` length in CSS (arbitrary utility, stylesheet rule, inline style) other than a hairline border or a media-query breakpoint | Rewrite in `rem` at `1rem = 16px` | (inline -- just convert) |
| Component-specific selector or `@apply` in the theme stylesheet | Move declarations into the consuming component as utilities on its markup; delete the selector | `colocate-component-styles.md` |
