...
Conduct the interview in **rounds**. Each round asks 1-3 focused questions using the platform's structured user-decision capability when available, or direct questions otherwise. Adapt questions based on previous answers and story context.
...
**Save to:** the `interview.md` of the folder the story resolved to — `ProductSpecification/stories/NN-story-name/`, or `stories/done/NN-story-name/` for a story written back into after it closed (`.claude/rules/workflow.md`, "Resolving a story folder").
...
"Resolving a story folder"). Interviewing a story that already shipped, in a second
folder beside its archived original, splits its rationale across two places.
