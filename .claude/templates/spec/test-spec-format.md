...
Stories declare prerequisites (e.g., "board must exist", "column must exist"). Each prerequisite MUST produce an API guard plus a blocker scenario in every relevant user-client category (`02_UI_Tests.md` and/or `07_Bot_Tests.md`).
...
## 07_Bot_Tests.md (3-6 tests)

Generate this file only when the story exposes behavior through the bot or changes an
API contract the bot consumes. Otherwise omit it and set `Bot = n/a` in `stories.md`.

Order scenarios by the conversation path they unlock:
1. Entry command or callback and prerequisite blockers
2. Input validation and recovery without losing conversation state
3. Happy path with the exact user-visible reply and API interaction
4. Multi-step state transitions and cancellation
5. API rejection, authorization, and transport-unavailable responses

Use the bot profile's isolated E2E harness. Assert observable bot output and the exact
API request; never inspect API storage. A flow against a real API belongs in
`06_Integration_Tests.md`, not this category.
...
4. **Write scenarios in Russian** — section headings, scenario titles and step text are Russian prose; structural keywords use the Russian Gherkin localization (`Дано`, `Когда`, `Тогда`, `И`). The examples throughout this template are English only because this template is framework prose (`.claude/rules/workflow.md`, "Language of Artifacts") — copy their *shape*, not their language.
5. End each file with DSL Technical Reference table. The left column is the Russian DSL statement as it appears in the scenarios; the right column stays in the wire contract's own terms:
...
|---------------|-------------------------|
| `авторизованный пользователь` | Valid JWT in Authorization header |
| `пользователь отправляет регистрацию` | POST /api/auth/register |
```
