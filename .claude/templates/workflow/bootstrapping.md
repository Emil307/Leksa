...
1. **Detect spec artifacts** in the story directory:
   ...
   - `test-spec`: at least one `tests/NN_*.md` category file exists
   ...
   **If `tests/` does not exist yet**, `test-spec` has not run: emit only the
   ...
   with no markers).
2. **Read the test specs** — `tests/NN_*.md`, whichever of the seven categories the
...
7. For new bot scenarios emit the three staged bot checkboxes from
   `parallel-bot-stages.md`. Preserve a custom bot shape when execution already
   started under it.
...
of `.claude/rules/workflow.md`: `01_API`, `06_Integration`, `02_UI`, `07_Bot`,
`05_Security`, `03_Load`, `04_Infrastructure` — the same order in each tier.
