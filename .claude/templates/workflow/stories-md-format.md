...
`# | Story | Spec | Back | Intg | Front | Bot | Sec | Load | Infra | Tests | %`. The
eight phase columns are `Spec` plus the seven test categories (`Back`, `Intg`, `Front`,
`Bot`, `Sec`, `Load`, `Infra`).
...
- **The seven test-category phase columns** (`Back`, `Intg`, `Front`, `Bot`, `Sec`,
  `Load`, `Infra`) split on `/` into per-tier status, `T1/T2` (e.g. `✅/🔧` = Tier 1 done, Tier 2
...
- **`Spec` is never split.** The `## Spec` section is untiered and holds no `### `
  ...
  `✅`, not "not applicable". Only the seven test-category columns are split.
...
**Lifecycle ordering**: Spec → Backend → Integration → Frontend → Bot → Security → Load →
