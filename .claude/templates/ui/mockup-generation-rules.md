...
- CSS lengths in `rem` (`1rem = 16px`), never `px` -- except `1px`/`2px` borders and media-query breakpoints. The frontend copies mockup values verbatim, so a `px` in the mockup becomes a `px` in production.
