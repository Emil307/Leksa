---
name: test-spec
description: Generate BDD test specifications for a story in 7 categories (API, Integration, UI, Bot, Security, Load, Infrastructure). Use when the user wants to create test cases or mentions /test-spec.
---

# Generate Test Specifications
Generate BDD-style test specifications for a story in 7 categories — one file per
...
- Business rules and constraints → map to API and relevant user-client scenarios
...
**Prerequisite analysis** (mandatory): Read the story's Prerequisites section and Validation Rules table. For each prerequisite, generate the API guard and client blocker scenarios in every relevant UI or Bot category following the Prerequisite Guard Checklist in `test-spec-format.md` — its rules cover extraction, coverage, and which two sources an established blocker pattern is read from — never a sweep of the other story folders.
...
Create the applicable files in the `tests/` directory of the folder the story resolved to — `ProductSpecification/stories/NN-story-name/tests/`, or `stories/done/NN-story-name/tests/` for a story that has closed (`.claude/rules/workflow.md`, "Resolving a story folder"):
- `01_API_Tests.md`, `02_UI_Tests.md`, `03_Load_Tests.md`
- `04_Infrastructure_Tests.md`, `05_Security_Tests.md`, `06_Integration_Tests.md`, `07_Bot_Tests.md`
...
## Rules
- Russian, Gherkin in Markdown, DSL only (no technical details in steps). Scenario titles and step text are Russian prose; structural keywords use the Russian Gherkin localization (`Дано`, `Когда`, `Тогда`, `И`). The DSL Technical Reference table keeps its right-hand column in the wire contract's own terms (methods, paths, status codes). See `.claude/rules/workflow.md`, "Language of Artifacts"
