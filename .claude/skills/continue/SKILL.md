...
| `stage-1 bot acceptance RED + interface design` | **Inline coordinator.** Load `parallel-bot-stages.md`; dispatch bot acceptance RED and `/design-preview` bot concurrent mode with disjoint manifests, join once, run combined checks, then stage and commit explicit paths |
| `stage-2 bot implementation lanes` | **Inline coordinator.** Load `parallel-bot-stages.md`; validate frozen interfaces, dispatch every non-trivial `bot-logic`, `bot-api`, and `bot-handler` lane concurrently, preserve RED-before-GREEN, join once, run combined checks, then commit explicit paths |
| `stage-3 bot acceptance GREEN + review` | **Inline coordinator.** Load `parallel-bot-stages.md`; dispatch remove-marker-only bot acceptance GREEN with both review agents over Stage 1+2, join once, then triage; admitted cycles become consent checkpoints |
...
`parallel-backend-stages.md`, `parallel-frontend-stages.md`, or
`parallel-bot-stages.md` template for lane
