...
### Concurrent Bot Interface-Design Mode

When Stage 1 of `parallel-bot-stages.md` dispatches this skill beside bot acceptance
RED, infer the handler, bot-logic, typed API-client, model, FSM, and test interfaces.
Declare disjoint Stage 2 manifests and create interface surfaces only.

Do not implement behavior, stage, commit, edit `progress.md`, or wait for the
acceptance lane. Return frozen paths, lane manifests, and combined checks. Reject
writable overlap or a dependency between Stage 2 lanes.
...
2. **Read the scenario** from the category file named by its progress section:
   `01_API`, `06_Integration`, `02_UI`, `07_Bot`, `05_Security`, `03_Load`, or
   `04_Infrastructure`
