...
**Every artifact this framework generates is written in Russian.** That is the story spec and its notes, `interview.md`, `endpoints.md`, `decisions/*-decision.md`, `progress.md` checkbox text, journey summaries, `tests/*.md` scenarios, mockup interface text, and every report shown to the user. A skill or template that names a language for its own output names Russian; where none is named, this rule governs.

**What stays English**, because it is read by a compiler, a parser, or a diff — never by a reader as prose:

| Stays English | Why |
|---|---|
| Code identifiers, file and folder names, branch names | Project convention (`CLAUDE.md`); a Russian identifier breaks tooling |
| Migration comments | Project convention |
| Commit-message subject and body | Repository history is already English; a mixed log is worse than either language |
| HTTP methods, paths, status codes, header names, JSON field names in `endpoints.md` and the DSL Technical Reference | They are the wire contract, quoted verbatim |
| Gherkin structural keywords in generated test scenarios | Use the Russian localization — `Дано`, `Когда`, `Тогда`, `И` — which every Gherkin parser accepts; scenario titles and step text are Russian prose |

**Framework prose itself — `.claude/**` rules, guidelines, skills, templates, agents — stays English.** It is merged from upstream by `/framework-sync`, and a translated local copy turns every future merge into a conflict. The rule governs what the framework *writes*, not what the framework *is*.

## Lifecycle

Every story follows: **interview → spec → backend scenarios → integration scenarios → frontend scenarios → bot scenarios → security scenarios → load scenarios → infrastructure scenarios**.

Those seven scenario types are the **categories**, not the delivery order. In a tier-major story, delivery runs **tier-first**: all of Tier 1 in that category order, then the `harvest` boundary, then all of Tier 2 in the same order. The boundary is the story's **shippable milestone** — when every Tier 1 scenario is done the feature works for its primary user, which is what all-left-`✅` phase cells mean; Tier 2 hardens what already works. Why tier outranks category is in `.claude/guidelines/workflow-detail.md` ("Why Delivery Is Ordered by Tier").

**High-level progress** is tracked in `ProductSpecification/stories.md` — three tables: **In Progress**, **Backlog**, and **Done**. Phase columns (Spec, Backend, Integration, Frontend, Bot, Security, Load, Infra) per story. The `/continue` skill updates it after each work unit commit. Phase values: ✅ done, 🔧 in progress, — not started, · no story folder yet. When **every checkbox in the story's `progress.md` is `[x]` or `[S]`** — not merely when Tests/% read full, which ignore non-scenario checkboxes like `harvest` — move its row from the **In Progress** table to the **Done** table **and, in the same commit, move its folder** `ProductSpecification/stories/NN-story-name/` → `ProductSpecification/stories/done/NN-story-name/`, so `ls stories/` keeps telling you what is in flight (see `.claude/templates/workflow/stories-md-format.md`). The archived folder is still read — its decisions and interview are where a closed story's rationale lives — so every lookup resolves both locations.
